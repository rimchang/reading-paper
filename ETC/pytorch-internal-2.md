...
In the first [post](https://gist.github.com/killeent/4675635b40b61a45cac2f95a285ce3c0) I explained how we generate a `torch.Tensor` object that you can use in your Python interpreter. Next, I will explore the build system for PyTorch. The PyTorch codebase has a variety of components:

첫번째 post 에서는 Python 인터프리터에서 사용할 수 있는 `torch.Tensor` object를 생성하는 방법을 알아봤습니다. 다음으로는 Pytorch 빌드시스템을 살펴봅니다. Pytorch 에는 다양한 구성요소가 있습니다.
 ...
 - 핵심 torch 라이브러리 : TH, THC, THNN, THCUNN
 - Vendor 라이브러리 : CuDNN, NCCL
 - Python extension 라이브러리
 - 추가적인 third-party 라이브러리 : Numpy, MKL, LAPACK
...
어떻게 `python setup.py install`와 같은 간단한 명령어를 통해 `import torch` 를 호출하고 Pytorch 라이브러리를 사용할 수 있도록 합니까?
...
이 문서의 첫번째 부분에서는 빌드 프로세스와 end-user 에서의 관점을 설명합니다. 위와 같은 구성요소를 사용하여 어떻게 라이브러리를 빌드하는지를 설명합니다. 이 문서의 두번째 부분은 Pytorch 개발자에게 매우 중요합니다. 코드의 일부분만을 작성함으로써 반복의 속도를 향상시키는 방법을 설명합니다.
...
Python은 Setuptools를 사용하여 라이브러리를 빌드합니다. Setuptools는 핵심 Python 라이브러리의 원래 distutils 시스템을 확장 한 것입니다. Setuptools의 핵심 구성 요소는 프로젝트를 빌드하는 데 필요한 모든 정보가 들어있는`setup.py` 파일입니다. 가장 중요한 함수는 `setup()` 함수입니다. 이는 main entry point를 제공합니다.
...
The function is composed entirely of keyword arguments, which serve two purposes: 
 ...
 1. Metadata (e.g. name, description, version)
 2. The contents of the package

이 함수는 전부 keyword argument로 구성이 되며 두가지 용도로 사용됩니다.

1. 이름, 설명, 버젼과 같은 Metadata
2. 패키지의 내용
...
우리는 # 2에 관심이 있습니다. 개별 구성 요소를 분해합시다.

 - ** ext_modules ** : 파이썬 모듈은 파이썬 코드 만 포함 된 "순수"모듈이거나 파이썬 구현의 하위 레벨 언어로 작성된 "확장"모듈입니다. 여기에 파이썬 텐서가 들어있는 주요`torch._C` 라이브러리를 포함하여 빌드에서 확장 모듈을 나열합니다.
 - ** cmdclass ** : 명령 줄에서`setup.py` 스크립트를 사용할 때 사용자는 하나 이상의 "명령"을 지정해야합니다.이 명령은 특정 작업을 수행하는 코드 스 니펫입니다. 예를 들어, "install"명령은 패키지를 빌드하고 설치합니다. 이 매핑은 특정 명령을 구현하는`setup.py`의 함수에 특정 명령을 전달합니다
 - ** 패키지 ** : 프로젝트의 패키지 목록. 이들은 "순수"합니다. 즉, 파이썬 코드 만 포함합니다. 이들은`setup.py`의 다른 곳에 정의되어 있습니다.
 - ** package_data ** : 패키지에 설치해야하는 추가 파일 :이 경우 빌드가 생성하는 헤더 파일과 공유 라이브러리는 설치시 포함되어야합니다
 - ** install_requires ** : PyTorch를 빌드하기 위해서는 pyyaml이 필요합니다. Setuptools는 pyyaml을 사용할 수 있는지 확인하고 필요한 경우 다운로드하고 설치합니다.

우리는 이러한 구성 요소를보다 자세히 고려할 것입니다. 그러나 지금은 설치의 최종 제품, 즉 코드를 빌드 한 후에 Setuptools가 수행하는 작업을 살펴 ​​보는 것이 좋습니다.
